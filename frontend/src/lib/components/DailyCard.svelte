<script lang="ts">
  ...
  $: colorClass = STATUS_COLOR[daily.status] ?? 'text-gray-400';
  $: bgClass = STATUS_BG[daily.status] ?? 'bg-gray-800/20 border-gray-700/30';
</script>

<div class="card border rounded-xl p-5 flex flex-col gap-4 {bgClass}">
  <div class="card-header">
    <span class="label">TODAY</span>
    <span class="sentiment {colorClass}">{daily.sentiment}</span>
    <span class="status {colorClass}">{daily.status.toUpperCase()}</span>
  </div>

  {#if daily.meme}
    <div class="meme-container">
      <img
        src="{API_URL}/memes/{daily.meme}"
        alt="{daily.status} meme"
        class="meme-img"
        loading="lazy"
      />
    </div>
  ...
  <p class="message">{daily.message || 'No analysis available yet.'}</p>

  {#if daily.evidence?.length > 0}
    <div class="section">
      ...
      <div class="posts-list">
        {#each daily.top_posts as post}
          <div class="post-item">
            ...
            {/if}
            <p class="post-meta">
              <span class="{STATUS_COLOR[post.status] ?? 'text-gray-400'}">{post.status}</span>
