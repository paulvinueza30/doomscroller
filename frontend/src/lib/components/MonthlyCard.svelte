<script lang="ts">
  ...
  $: colorClass = STATUS_COLOR[monthly.status] ?? 'text-gray-400';
  $: bgClass = STATUS_BG[monthly.status] ?? 'bg-gray-800/20 border-gray-700/30';
</script>

<div class="card border rounded-xl p-5 flex flex-col gap-4 {bgClass}">
  <div class="card-header">
    <span class="label">THIS MONTH</span>
    <span class="sentiment {colorClass}">{monthly.sentiment}</span>
    <span class="status {colorClass}">{monthly.status.toUpperCase()}</span>
  </div>

  {#if monthly.meme}
    <div class="meme-container">
      <img
        src="{API_URL}/memes/{monthly.meme}"
        alt="{monthly.status} meme"
        class="meme-img"
        loading="lazy"
      />
    </div>
  ...
  <p class="message">{monthly.message || 'No analysis available yet.'}</p>

  {#if monthly.evidence?.length > 0}
    <div class="section">
      <h3 class="section-title">Evidence</h3>
      <ul class="evidence-list">
        {#each monthly.evidence as phrase}
          <li>• {phrase}</li>
        {/each}
      </ul>
    </div>
  ...
  <Chart data={history} title="Last 30 Days" />

  {#if monthly.top_posts?.length > 0}
    <div class="section">
      <h3 class="section-title">Top Posts</h3>
      <div class="posts-list">
        {#each monthly.top_posts as post}
          <div class="post-item">
